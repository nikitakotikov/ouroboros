...
function initChat() {
    ...
    page.innerHTML = `
        ...
        <div id="chat-input-area">
            ...
            </button>            <button class="icon-btn" id="chat-upload" title="Upload file">                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>            </button>            <input type="file" id="chat-file-input" style="display:none" accept=".csv,.xlsx,.xls,.json,.txt,.md">
        </div>
    `;
        ...
        fileInput.addEventListener('change', async () => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (result.success) {
                        addMessage(`📤 Uploaded: ${result.filename} (${formatFileSize(result.size)})`, 'assistant');
                    } else {
                        addMessage(`❌ Upload failed: ${result.error}`);
                    }
                } else {
                    addMessage(`❌ Upload failed: ${response.statusText}`);
                }
            } catch (err) {
                addMessage(`❌ Upload error: ${err.message}`, 'assistant');
            } finally {
